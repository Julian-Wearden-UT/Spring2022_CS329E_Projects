...
function formPage(){
    ...
    <head>
        <meta name="description" content="Update Student Record">
        <meta charset="UTF-8">
    ...
    <body>
        <form method="POST" action="$script" id="form">
            ...
            <p>
                ...
                <input name = "Submit" id="validate" class = "validate" type = "button" value = "Submit" onclick="validateUpdate()">
            </p>
        </form>
...
function outputResults(){
    ...
    if(isset($_POST["majorField"]) && $_POST["majorField"] != ""){
        $major = $_POST["majorField"];
        $command = "UPDATE STUDENTS SET MAJOR = \"$major\" WHERE ID = $id;";
        $result = $mysqli->query($command);
    }
    if(isset($_POST["gpaField"]) && $_POST["gpaField"] != ""){
        $gpa = $_POST["gpaField"];
        $gpa = (float)$gpa;
    ...
    if (!$result) {
        ...
        print <<< UPDATE
        <script>document.getElementById('Result').innerText = "Student $id has been updated"</script>
UPDATE;
    }
}
?>
